<template>
  <div class="space-y-4">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="space-y-4">
        <h3 class="font-medium text-lg">TSV/CSV 转换器</h3>

        <div class="space-y-3">
          <div>
            <label class="block text-sm font-medium mb-1">输入文本</label>
            <textarea v-model="inputText" rows="10" class="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="粘贴 CSV 或 TSV 文本..." />
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2">
              <div>
                <label class="block text-xs text-gray-500 mb-1">源分隔符</label>
              ...
              <div>
                <label class="block text-xs text-gray-500 mb-1">目标分隔符</label>
              ...
              <div>
                <label class="block text-xs text-gray-500 mb-1">引用引号</label>
            ...
            <div class="grid grid-cols-2 gap-2 mt-2">
              ...
              <div class="flex items-center h-[42px] px-3 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                ...
                <label for="trimCell" class="text-sm">裁剪单元格空白</label>
              </div>
            </div>
          </div>
      ...
      <div class="space-y-4">
        ...
        <div v-if="result" class="space-y-4">
          <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
            ...
            <div class="mt-3 text-xs text-gray-500 space-y-1">
              ...
              <div>行数：{{ stats.rows }} · 列数：{{ stats.cols }}</div>
            </div>
          </div>
        ...
        <div v-if="!result" class="text-center py-12 text-gray-500 dark:text-gray-400">
          ...
          <div class="text-lg mb-2">TSV/CSV 转换</div>
          <div class="text-sm">在逗号与制表符分隔之间转换</div>
        </div>
      ...
      <div class="space-y-2 max-h-48 overflow-y-auto">
        <div v-for="(item, i) in history" :key="i" class="bg-gray-50 dark:bg-gray-800 rounded p-3 text-sm">
          <div class="flex justify-between items-start mb-2">
            <div class="font-medium">行 {{ item.rows }} · 列 {{ item.cols }}</div>
    ...
    <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
      ...
      <div class="text-sm text-yellow-800 dark:text-yellow-200 space-y-1">
        <div>• 自动检测会根据首行逗号与制表符出现次数判断</div>
        <div>• 导出 CSV 会对包含分隔符/换行/引号的单元格加引号并转义引号</div>
      </div>
    </div>
  </div>
</template>
