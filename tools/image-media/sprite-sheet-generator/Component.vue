<template>
  <div class="space-y-4">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div class="space-y-4">
        <h3 class="font-medium text-lg">雪碧图生成器</h3>

        <div class="space-y-3">
          <div>
            <label class="block text-sm font-medium mb-1">选择多张图片</label>
          ...
          <div class="flex gap-2">
            <button @click="process" :disabled="!items.length" class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-md">生成</button>
      ...
      <div class="space-y-4">
        ...
        <div v-if="outUrl" class="space-y-4">
          <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
            ...
            <div class="text-xs text-gray-500 mt-2">尺寸: {{ sheetMeta }}</div>
          </div>

          <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
            <div class="flex justify-between items-center mb-2">
              <h4 class="font-medium">Mapping JSON</h4>
              <div class="flex gap-2">
                ...
                <button @click="downloadJson" :disabled="!result" class="px-3 py-1 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white rounded text-sm">下载</button>
              </div>
            </div>
        ...
        <div v-else class="text-center py-12 text-gray-500 dark:text-gray-400">
          ...
          <div class="text-lg">选择多张图片并生成雪碧图</div>
        </div>
      ...
      <div class="space-y-2 max-h-48 overflow-y-auto">
        <div v-for="(h, i) in history" :key="i" class="bg-gray-50 dark:bg-gray-800 rounded p-3 text-sm">
          <div class="flex justify-between">
            <div class="font-medium truncate">图片: {{ h.count }} · 列数: {{ h.cols }}</div>
            <div class="text-xs text-gray-500">{{ formatDate(h.timestamp) }}</div>
          </div>
          <div class="text-xs truncate">尺寸: {{ h.sheetMeta }}</div>
