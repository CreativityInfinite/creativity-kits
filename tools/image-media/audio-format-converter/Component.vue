<template>
  <div class="space-y-4">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">{{ $t('tools.audio-format-converter.page.title') }}</h1>
      <p class="text-gray-600 dark:text-gray-400">{{ $t('tools.audio-format-converter.page.description') }}</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        <label class="block text-sm font-medium mb-2">{{ $t('tools.audio-format-converter.page.selectAudio') }}</label>
      ...
      <div>
        <label class="block text-sm font-medium mb-2">{{ $t('tools.audio-format-converter.page.targetFormat') }}</label>
        <select v-model="format" class="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white">
          <option value="wav">{{ $t('tools.audio-format-converter.page.wavLossless') }}</option>
          <option value="mp3">{{ $t('tools.audio-format-converter.page.mp3Lossy') }}</option>
        </select>
      </div>
      <div v-if="format === 'mp3'">
        <label class="block text-sm font-medium mb-2">{{ $t('tools.audio-format-converter.page.bitrateKbps') }}</label>
    ...
    <div class="flex justify-center gap-3">
      <button @click="convert" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md" :disabled="loading">
        {{ loading ? $t('tools.audio-format-converter.page.processing') : $t('tools.audio-format-converter.page.start') }}
      </button>
      <button v-if="outUrl" @click="download" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-md">{{ $t('tools.audio-format-converter.page.download') }}</button>
    </div>
    ...
    <div v-if="audioUrl || outUrl" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border">
        <div class="text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">{{ $t('tools.audio-format-converter.page.originalAudio') }}</div>
        <audio :src="audioUrl" controls class="w-full"></audio>
      </div>
      <div class="bg-white dark:bg-gray-800 rounded-lg p-4 border">
        <div class="text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">{{ $t('tools.audio-format-converter.page.convertResult') }}</div>
...
async function convert() {
  ...
  outUrl.value = '';
  if (!file.value) {
    error.value = t('tools.audio-format-converter.page.selectAudioFirst');
    return;
  }
  loading.value = true;
  try {
    ...
    error.value = e?.message || t('tools.audio-format-converter.page.convertFailed');
  } finally {
    loading.value = false;
  }
}
