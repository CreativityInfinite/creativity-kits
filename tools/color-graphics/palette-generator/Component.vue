<template>
  <div class="space-y-6">
    <div class="text-center">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">调色板生成器</h1>
      <p class="text-gray-600 dark:text-gray-400">从图片中提取主色调（本地处理，不上传），可设置颜色数量并导出 JSON</p>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"> 选择图片并设置参数 </label>
          <input
            type="file"
            accept="image/*"
            @change="onFile"
          ...
          <div class="grid grid-cols-2 gap-2 mt-3">
            <div>
              <label class="block text-xs text-gray-500 mb-1">颜色数量 (K)</label>
            ...
            <div>
              <label class="block text-xs text-gray-500 mb-1">采样步长（像素）</label>
          ...
          <div v-if="colors.length" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 mt-3">
            <div v-for="(c, i) in colors" :key="i" class="rounded overflow-hidden border dark:border-gray-700">
              ...
              <div class="px-2 py-1 text-xs flex items-center justify-between">
                ...
                <button @click="copyText(c.hex)" class="px-1 py-0.5 bg-gray-700 hover:bg-gray-800 text-white rounded text-[10px]">复制</button>
              </div>
            </div>
          </div>
        </div>
