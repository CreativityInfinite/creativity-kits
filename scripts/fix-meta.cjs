...
function getSegs(file) {
  // .../FancyTools/tools/<category>/<id>/meta.ts
...
function ensureMeta(file) {
  ...
  if (typeof color === 'string' && color) lines.push(`  color: '${color}',`);
  if (typeof icon === 'string' && icon) lines.push(`  icon: '${icon}',`);
  ...
  // ensure no trailing commas before closing }
